...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _send_response(self, status_code, message):
        ...
        self.send_header('Access-Control-Allow-Origin', 'http://qacemnba.indegenenextoci.com')
        ...
        if self.path == '/HCP':
            ...
            try:
                ...
                if 'calls' in action_param:
                    ...
                    rep_count = Counter(item['REP'] for item in dynamo_data)
                    
                    # Limit occurrences for each REP
                    for item in dynamo_data:
                        if rep_count[item['REP']] > rep_limit:
                            rep_count[item['REP']] -= 1
                            dynamo_data.remove(item)
